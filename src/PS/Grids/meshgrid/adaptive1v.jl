...
"""
  Refinement scheme: Local-Local-shape (fixed level with variable shape function)
...
function vchebyadaptive(fLnshape::Function,vGdom::Vector{T},j::Vector{Int}=[0,10],
    ...
    if j[1] < orderVconstllimit && j[2] < orderVconstllimit
        ...
        nck1, nvlevel1, vlevel1 = vchebyadaptive(fLnshape,vGdom,j,limitshape[1];nc0=nc0,vadaptlevels=vadaptlevels)
        if j[1] == j[2]
            if limitshape[2] ≥ limitshape[1] > 0
                ...
                for k in 1:nc0 - 1
                    if nvlevel1[k] .> limitshape[2]
                        for nc0sub in 3:5
                            ncksub, nvlevelsub, vlevelsub = vchebyHadaptive(fLnshape,[vlevel1[k][1],vlevel1[k][end]],
                                                   j[1],limitshape[1];nc0=nc0sub,vadaptlevels=vadaptlevels)
                ...
                if Nsub == 0
                    ...
                    # @show length(sub.nvlevel2), nc0new
                    for k in 1:nc0 - 1
                        if sub.nvlevel2[k] == 1
                            s += 1
                    ...
                    return nc0new,  (nck1, nvlevelre, vlevelre)
                end
            else
                return nc0, (nck1, nvlevel1, vlevel1)
            end
            ...
            if limitshape[2] ≥ limitshape[1] > 0
                ...
                for k in 1:nc0 - 1
                    if nvlevel1[k] .> limitshape[2]
                        for nc0sub in 3:5
                            ncksub, nvlevelsub, vlevelsub = vchebyadaptive(fLnshape,[vlevel1[k][1],vlevel1[k][end]],
                                                   j,limitshape[1];nc0=nc0sub,vadaptlevels=vadaptlevels)
                            sub.nvlevel2[k] = nc0sub - 1
                            sub.nvlevel2i[k] = ncksub
                            sub.nvlevel[k] = nvlevelsub
                            sub.vlevel[k] = vlevelsub
                            sum(nvlevelsub .> limitshape[2]) == 0 ? break : 1
                            @show ncksub, nvlevelsub
                        end
                    end
                end
                Nsub = sum(sub.nvlevel2 .- 1)
...
function vchebyadaptive(fLnshape::Function,vGdom::Vector{T},j::Vector{Int}=[0,10],
    ...
    if j[1] ≠ j[2]
        ~, nvlevelj1, vlevelj1 = vchebyadaptive(fLnshape,vGdom,j[1],limitshape;nc0=nc0,vadaptlevels=vadaptlevels)
...
"""
  ...
  inputs:
    limitshape: the lowest shape limit.
    vadaptlevels：The maximum level of adaptation for refinement grids: `nGki_re: 2^(vadaptlevels)+1`

  Outputs:
    nck, nvlevel, vlevel = vchebyadaptive(fLnshape,vGdom,j=orderVconst,limitshape=limitshape;nc0=nc0,vadaptlevels=vadaptlevels)
    nck, nvlevel, vlevel = vchebyadaptive(fLnshape,vGdom,0,9;nc0=nc0,vadaptlevels=vadaptlevels)
...
function vchebyadaptive(fLnshape::Function,vGdom::Vector{T},j::Int=2,
    ...
    limitshape < 0 ? ArgumentError(error("limitshape[1] < 0  is not permitted")) : 1
    ...
    if j < orderVconstllimit
        ...
        for i in 1:nc0 - 1
            ...
            for k in 1:vadaptlevels
                ...
                if min(δdMsk,10 * 2δdMsk / abs(dMsk + dMs2k)) < epsT5
                    nck += orderk - 2
                    vkup = vk
                    break
                else
            ...
            if limitshape > 0 && orderk < limitshape
