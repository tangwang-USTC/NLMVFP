...
function subdivsinitial(nc0::Int,nvlevel1::Vector{Int},vlevel1::AbstractVector{Any}) 
...
"""
  ...
  inputs:
    vG0: The initial grids, i.e., Chebyshev grids or uniform grids.
  ...
  Outputs:
    nck, nvlevel, vlevel = vchebyHadaptive(fLnshape,vG0,j=jVconst;nc0=nc0,ocp=ocp,abstol=abstol,reltol=reltol,vadaptlevels=vadaptlevels)

"""

# [j,level,nv], (2,1) Single-shape limits (upper) and double conservation of order: `[j_lower,j_upper]`. level = 1
function vchebyHadaptive(fLnshape::Function,vG0::AbstractVector{T},j::Vector{Int}=[0,2];
    nc0::Int=15,ocp::Int=3,abstol::Float64=epsT5,reltol::Float64=1e-5,
    vadaptlevels::Int=6,jVconstlimit::Int=1) where{T}

    if j[1] ≠ j[2]
        ~, nvlevelj1, vlevelj1 = vchebyHadaptive(fLnshape,vG0,j[1];
                      nc0=nc0,ocp=ocp,abstol=abstol,reltol=reltol,
                      vadaptlevels=vadaptlevels,jVconstlimit=jVconstlimit)
        nck1, nvlevel1, vlevel1 = vchebyHadaptive(fLnshape,vG0,j[2];
                      nc0=nc0,ocp=ocp,abstol=abstol,reltol=reltol,
                      vadaptlevels=vadaptlevels,jVconstlimit=jVconstlimit)
        for k in 1:nc0 - 1
        ...
        # nck, nvlevel, vlevel1 = vchebyHadaptive(fLnshape,vG0,j[1];
                    #   nc0=nc0,ocp=ocp,abstol=abstol,reltol=reltol,
                    #   vadaptlevels=vadaptlevels,jVconstlimit=jVconstlimit)
        return vchebyHadaptive(fLnshape,vG0,j[1];nc0=nc0,ocp=ocp,abstol=abstol,reltol=reltol,
                                vadaptlevels=vadaptlevels,jVconstlimit=jVconstlimit)
    end
end
...
"""
  ...
  inputs:
    ...
    v: The initial grids, i.e., Chebyshev grids or uniform grids.
    reltol: default = 1e-5, the relative tolerance when  `abstol < epsT5`
    vadaptlevels：The maximum level of adaptation for refinement grids: `nGki_re: 2^(vadaptlevels)+1`

  Outputs:
    nck, nvlevel, vlevel = vchebyHadaptive(fLnshape,vG0,j=jVconst;
                  nc0=nc0,ocp=ocp,abstol=abstol,reltol=reltol,vadaptlevels=vadaptlevels)

"""

# [level,nv], (1,1) Single-shape limits (upper) and single conservation of order: `j`. level = 1
function vchebyHadaptive(fLnshape::Function,v::AbstractVector{T},j::Int=2;
    nc0::Int=15,ocp::Int=3,abstol::Float64=epsT5,reltol::Float64=1e-5,
    vadaptlevels::Int=6,jVconstlimit::Int=1) where{T}

    vlevel = Vector((undef),nc0-1)
    # Whether refining is done according to `j` and `jVconstlimit` based on the integrals
    if vadaptlevels == 0 || j < jVconstlimit
        for k in 1:nc0 - 1
            vlevel[k] = [v[k], v[k+1]]
        end
        ...
        for k in 1:nc0 - 1
            ...
            if vadaptlevels == 1 || isconverged == -1
                ...
                # @show k, isconvergedk
                if vadaptlevels == 2
                    ~, vk = evalrulec(fLnshape,vk,nv,j,isconvergedk;ocp=ocp,abstol=abstol,reltol=reltol)
                    nvlevel[k] = length(vk)
                    vlevel[k] = deepcopy(vk)
                    # Nlevel = 2
                    # @show k, Nlevel, nv, isconvergedk1
                # elseif vadaptlevels == 3
                #     isconvergedk, vk = evalrulec(fLnshape,vk,nv,j,isconvergedk;ocp=ocp,abstol=abstol,reltol=reltol)
                #     # Nlevel = 2
                ...
                #     vlevel[k] = deepcopy(vk)
                    ...
                    while prod(isconvergedk) == 0
                        if Nlevel ≥ vadaptlevels
                            @warn ("Maximum number of level for refinement is reached before converging! k=",k)
                            break
                        end
                        isconvergedk, vk = evalrulec(fLnshape,vk,nv,j,isconvergedk;ocp=ocp,abstol=abstol,reltol=reltol)
                        nv = length(vk)
...
"""
  ...
  Outputs:
    isconverged, vk = evalrulec(fLnshape,vkdom,nv,j,isconverged;ocp=ocp,abstol=abstol,reltol=reltol)
"""

# for `H-adaptive` refinement or `1/4-adaptive` for subdivision [vk,vk1]
function evalrulec(fLnshape::Function,vkdom::Vector{T},nv::Int,j::Int,
    ...
    for k in 1:nv-1
        if isconverged[k] == 1          # no refinement, only the endpoints is needed
            ...
            isconverged[k], vk[k] = evalrulec(fLnshape,vkdom[k:k+1],j,isconverged[k];ocp=ocp,abstol=abstol,reltol=reltol)
            nvlevel[k] = ocp
            if isconverged[k] == 1
...
"""
  ...
  Outputs:
    isconverged, vk = evalrulec(fLnshape,v2dom,j,isconverged;ocp=ocp,abstol=abstol,reltol=reltol)

"""

function evalrulec(fLnshape::Function,v2dom::Vector{Tb},j::Int=2,
    isconverged::Int=0;ocp::Int=3,abstol::Float64=epsT5,reltol::Float64=1e-5) where{Tb}
...
function nvlevel0invGk(nvlevel0,vG0,vlevel,nc0,nck)
    ...
    if nc0 == nck
        ...
        prod(nvlevel0 .≠ 0) == 0 ? @error("position vector of `vG0` in `vGk` is failure !!!") : 1
        return nvlevel0
    end
end
