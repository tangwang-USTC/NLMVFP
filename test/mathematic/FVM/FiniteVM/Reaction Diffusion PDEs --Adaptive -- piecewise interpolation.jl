...
max_area_function = (A, r) -> Amin * tot_area * r^order / A
...
fig3 = Figure(fontsize=38)
for i in eachindex(sol)
    ax = Axis(fig3[1, i], width=400, height=400,
        xlabel="x", ylabel="y",
        title="t = $(sol.t[i])",
        titlealign=:left)
...
fig4 = Figure(fontsize=55, resolution=(6350, 1550)) # resolution from resize_to_layout!(fig4) - had to manually adjust to fix missing ticks
for (i, (vals, title)) in enumerate(zip(all_vals, titles))
    ax2d = Axis(fig4[1, i], xlabel="x", ylabel="y", width=600, height=600, title=title, titlealign=:left)
    ax3d = Axis3(fig4[2, i], xlabel="x", ylabel="y", width=600, height=600, title=title, titlealign=:left)
