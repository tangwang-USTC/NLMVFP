...
#                                 ## ------------------------------------------------------------------------------------------------------------------------
orderRK, rsRK, iterRK = 2, 2, 7                 # [:Euler(1,:,N), :Trapz(2,:,N⁺), :RadauI3(3,2,N), :GL4(4,2,N), :LobattoIIIA4(4,3,N), :LobattoIIIA6(6,4,N), ⋯]
...
Nτ_fixvec = [5 * 2 .^1]
if is_MultiCase
    NCasennv = length(nnvocpM)    # (nnv0, ocp0) = [(6,6), (7,7), (8,8)]
    ...
    # Cnm_name = :C1        # `:C1: RDKingvec = (Kingk1 - flnk1) / (Δt * fvLc0k1[isp])`
    is_weight_DKing = true
    if Cnm_name == :C0
    ...
    if iCase == 1
        ...
        NNτ_fixvec = length(Nτ_fixvec) 
        is_save_nvG_NCase = false
        if is_save_nvG_NCase
            RDKingvec = zeros(nvG[1]+1,NNτ_fixvec,2)
        else
            RDKingvec = zeros(1+1,NNτ_fixvec,2)
        end
        if is_L2dtfl0
            fl0L2dtv = zeros(nvG[1],NNτ_fixvec)             # For the exact species
        end
        if is_L2dtKing
            KingL2dtv = zeros(nvG[1],NNτ_fixvec)            # For the exact species
        end
        if is_L2dtDKing
            DKingL2dtv = zeros(nvG[1],NNτ_fixvec+1)         # For the exact species
        end
        if is_CPUtime
            CPUvec = zeros(NNτ_fixvec+1)                    # [s]
        end
        if is_CPUnnvtime
            CPUnnvvec = zeros(NCasennv)                            # [s]
        end
    end
    if is_CPUtime 
        if iCase == NCase
    ...
    if is_CPUnnvtime && iCase == NCase
        ...
        CPUnnvvec1order = CPUnnvvec[NCasennv] .* (2^1) .^(0:NCasennv-1)
        CPUnnvvec1order *= CPUnnvvec[NCasennv] / CPUnnvvec1order[NCasennv]
...
################################# For timestep
Nτ_fix = 8 * 2^3               # (=20, default) Number of timestep during one characteristic time `τ₀`
if is_MultiCase
    Nτ_fix = Nτ_fixvec[iCase]
    RDKing00vec[iCase,6] = 1 / Nτ_fix
end
rtol_DnIK = 0.05               # (=0.02, default) For the minimum timestep according to `abs(dt * Rc / Mc) ≤ rtol_DnIK`
...
atol_nuTi_optim = 1e-2         # (=1e-10, default), The criterion condition for optimization of `nai,uai,vthi`, or else optimization is falure.
