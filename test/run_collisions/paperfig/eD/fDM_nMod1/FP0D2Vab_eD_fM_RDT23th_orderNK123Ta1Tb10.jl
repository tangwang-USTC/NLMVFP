...
is_NKCnhC = false              # (=true, default)
if is_nai_const == true 
    is_NKCnhC = false
end
...
rtol_DnuTi = epsT10               # (=1e-3, default) for `:Ms` version. The criterion condition for characteristic values to reduce the number of `nMod`
...
Nv2ratio0 = Nv2ratio(nc0[1], 1, 16, max(vth[1], vth[2]), min(vth[1], vth[2]))
