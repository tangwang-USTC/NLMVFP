...
NK = 2                            # Number of King function 
NK_initial = 1
is_nai_const = true
is_optim_CnIK = false         # Enforceing conservations in a single spices during optimization by applying the conservative constraints to reduce the optimization equations.
# optim_strage = :NK      # [:naiC, :NK0, :NKf0, :NKC, NKfC]
is_fixed_NK = true        # (=false, default), fixing the parameter `NK` during time evolution.
atol_fM_Rdtsab = 1e-6
...
const vhthMin = 1e-5    # TD = 1 keV  ~  Tα = 3 MeV. The maximum absolute span of the submoments;
const vhthInitial = 5e-1# vthi
...
is_dt_unit = false              # true ? dt = 1 / Nτ_fix : (tspan[2] - tspan[1]) / Nτ_fix
################################# For timestep
...
Te1, Te2, Tp0, TD0, TT0, Tα0  = 5.0, 10.0, 1.0, 10.0, 11.0, 1000.0
