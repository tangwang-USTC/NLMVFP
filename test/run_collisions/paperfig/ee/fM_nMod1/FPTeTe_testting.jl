...
if is_MultiCase
    ...
    if iCase == 1
        RDTab33vecTaTb = zeros(NCase,2+1)
    end
end

DRDTab33vecTaTb = abs.(RDTab33vecTaTb[1:end-1,1:2] .- RDTab33vecTaTb[end,1:2]')
orderaTaTbvec = order_converg(DRDTab33vecTaTb[:,1])
orderbTaTbvec = order_converg(DRDTab33vecTaTb[:,2])
################################# For timestep
# Nτ_fix = 5 * 2^6               # (=20, default) Number of timestep during one characteristic time `τ₀`
