...
if isfile(file_Ms_Cerror)
    ...
    ppena = plot(tplot[tvec],a,line=(wline,:auto),label="a",
                    ylabel=ylabel,yscale=:log10)
    b = (abs.(Cerrort.edtnb[tvec]) .+ epsT01)
    if is_MultiCase
        eRdtnbM[iCase] = deepcopy(b)
    end
    ppenb = plot!(tplot[tvec],b,line=(wline,:auto),label="b")
    if ns ≥ 3
    ...
    ppeIha = plot(tplot[tvec],a,line=(wline,:auto),label="a",
                      ylabel=ylabel,yscale=:log10)
    b = (abs.(Cerrort.edtIb[tvec]) .+ epsT01)
    if is_MultiCase
        eRdtIbM[iCase] = deepcopy(b)
    end 
    ppeIhb = plot!(tplot[tvec],b,line=(wline,:auto),label="b")
    if ns ≥ 3
    ...
    ppeKha = plot(tplot[tvec],a,line=(wline,:auto),label="a",
                 ylabel=ylabel,yscale=:log10,
                 xlabel="t")
    ...
    ppeKhb = plot!(tplot[tvec],b,line=(wline,:auto),label="b")
    if ns ≥ 3
    ...
    ppeKa = plot(tplot[tvec],a,line=(wline,:auto),label="a",
                     ylabel=ylabel,yscale=:log10,
                     xlabel=title_model)
    ...
    ppeKb = plot!(tplot[tvec],b,line=(wline,:auto),label="b")
    if ns ≥ 3
    ...
    # ppeKa = plot(tplot[tvec][1:NTat-dN],a,line=(wline,:auto),label="a",
    #                  ylabel=ylabel,yscale=:log10,
    #                  xlabel=title_model)
    # b = (abs.(Cerrort.edtKb[tvec][1:NTat-dN] .* Tbt[tvec[1:NTat]][1:NTat-dN]) .+ epsT01)
    # ppeKb = plot!(tplot[tvec][1:NTat-dN],b,line=(wline,:auto),label="b")
    # if ns ≥ 3
    ...
    ppeTa = plot(tplot[tvec],a,line=(wline,:auto),label="a",
                ylabel=ylabel,
                xlabel="t")
    b = (abs.(Cerrort.eRdtTb[tvec]) .+ epsT01)
    ppeTb = plot!(tplot[tvec],b,line=(wline,:auto),label="b")
    if ns ≥ 3
