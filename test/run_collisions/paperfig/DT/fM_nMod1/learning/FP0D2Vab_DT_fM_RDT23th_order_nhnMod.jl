...
is_nai_const = true
is_nhnMod_adapt = false
is_optim_CnIK = false          # (=true, default) Enforceing conservations in a single spices during optimization 
...
is_NKC_vhthInitial = true     # (=true, default) which denotes `? vhrhi[i] = vhthInitial : vhrhi[i] = vthi * vhthRatio`
