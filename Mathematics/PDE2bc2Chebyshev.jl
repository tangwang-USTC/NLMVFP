"""
   ...
   S.C. Reddy, J.A.C. Weideman  1998 -- Matlab
...
function chebyshevbc(n::Int,g::AbstractVector{T};datatype::DataType=Float64) where{T,N}
   ...
   elseif b1≠0 && b9==0 # Dirichlet x=1, Robin x=-1
      ...
      ϕ9 = (xkcol .* D1[K,n] - D0[K,n]) / 2     # phi'_-, phi''_-
...
"""
  Inputs:
    n       = nc0, the number of Chebyshev points in `domain = [-1,1]`
    x, Dc12 = chebyshevdiff(x,n; M = 2,datatype=datatype)
    vG      = vCmapping(x,vGdom[1],vGdom[2];isinv=true)
    g      =  boundary condition matrix = `[a1, b1, c1, a9, b9, c9]

  Outputs:
    vGt,D1t,D2t,ϕ1,ϕ9 = chebyshevbc(x,vG,D1,D2,g,n)
"""

function chebyshevbc(x::AbstractVector,vG::AbstractVector,D1::AbstractArray{T,2},D2::AbstractArray{T,2},
   g::AbstractVector{T},n::Int=length(x);datatype::DataType=Float64) where{T,N}
   ...
   elseif b1==0 && b9==0 # Dirichlet / Dirichlet
      ...
      # ϕ9 = c9 / a9 * [D1[K,n] D2[K,n]]          # ϕ-
      # xt = vG[K]                                  # node vector
      return vG[K],D1[K,K],D2[K,K],c1 / a1 * [D1[K,1] D2[K,1]],c9 / a9 * [D1[K,n] D2[K,n]]
   elseif b1≠0 && b9==0 # Dirichlet x=1, Robin x=-1
      ...
      # xt = vG[K]                             # node vector
      return vG[K],[fcol1 D1t],[fcol2 D2t],c1 / b1 * [ϕ1 (- xkcol .* D2[K,1]+2 * D1[K,1])],c9 / a9 * [ϕ9 xkcol .* D2[K,n] / 2 - D1[K,n]]
      return xt,D1t,D2t,ϕ1,ϕ9
   elseif b1==0 && b9≠0 # Case 3: neumann or Robin boundary x=1 and Dirichlet at x=-1.
      ...
      cfac = D1[n,n] + a9 / b9                # compute phi'_n, phi''_n
      lcol1 = - cfac * D0[K,n] + (1 .- cfac * xkcol) .* D1[K,n]
      lcol2 = - 2cfac * D1[K,n] + (1 .- cfac * xkcol) .* D2[K,n]
      # D1t  = [D1t lcol1]
      # D2t  = [D2t lcol2]
      ...
      # xt = vG[K]                               # node vector
      return vG[K],[D1t lcol1], [D2t lcol2], c1 / a1 * [ϕ1 xkcol .* D2[K,1] / 2 + D1[K,1]], c9 / b9 * [ϕ9 xkcol .* D2[K,n] + 2 * D1[K,n]]
      # return xt,D1t,D2t,ϕ1,ϕ9
   elseif (b1≠0 && b9≠0) # Case 4: neumann or Robin boundary conditions at both endpoints.
      ...
      # xt = vG[K]                                  # node vector
      return vG[K],[rcol1 D1t lcol1],[rcol2 D2t lcol2],c1 / b1 * [phip1 phip2],c9 / b9 * [phim1 phim2]
      return xt,D1t,D2t,ϕ1,ϕ9
   end
end
