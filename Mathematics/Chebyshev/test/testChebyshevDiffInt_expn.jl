...
##################           ChybyshevApprox
nG = 35              # Affect the Interpolations and differences
vdomain = [-1.0,1.0]
kind = 2
nc = nG
n = 3                # factor for function: `f(v) = v^n * exp^(-v^2)`
order = 31           # Affect the Interpolations and differences
                     #  but not the differences by matrices `Dc`

## Target Functions
# f0(v) = exp(-v^2)
# df0(v) = - 2v * exp(-v^2)
# If0(v) = ù / 2 * (erf(1.0) + erf(v))

f(v) = v^n * exp(-v^2)
df(v) = (n * v^(n-1) - 2 * v^(n+1)) * exp(-v^2)
If(v) = 0.5((-1)^(n+1) * gamma((n+1)/2,1) + (-1)^n * gamma((n+1)/2,v^2))

println()
printstyled("kind=",kind,",factor,n=",n,",nc=",nc,",order=",order,",vdom=",vdomain;color=:yellow)
println()
